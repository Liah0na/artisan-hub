  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/dashboard/profile", {
        ...
        body: JSON.stringify({ name, bio, phone, instagram, location, avatar: finalAvatar }),
      });
    ...
    <form onSubmit={handleSubmit} className="mt-8 max-w-2xl space-y-5 rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          ...
          <input id="phone" value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="+55 21 90000-0000" className="w-full rounded-md border px-3 py-2" />
        </div>
        <div>
          <label htmlFor="instagram" className="mb-2 block text-sm font-medium">Instagram</label>
          <input id="instagram" value={instagram} onChange={(event) => setInstagram(event.target.value)} placeholder="@seu_usuario" className="w-full rounded-md border px-3 py-2" />
        </div>
      </div>

      <div>
        <label htmlFor="location" className="mb-2 block text-sm font-medium">Cidade / localização</label>
        <input id="location" value={location} onChange={(event) => setLocation(event.target.value)} placeholder="Niterói, RJ" className="w-full rounded-md border px-3 py-2" />
      </div>
